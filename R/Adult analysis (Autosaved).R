...
xloc <- c(difftime(lp[,"start"], lp[,"dob"], unit="days"))/365.25
life <- c(difftime(lp[,"end"], lp[,"dob"], unit="days"))/365.25
cyststart <- c(lp[,"cyst"] - lp[,"dob"])/365
for (i in 1:length(yloc)) {
	segments(x0=xloc[i], y0=yloc[i], x1=life[i], y1=yloc[i], col="darkgray") # Life line
...
cox.zph(coxF, transform="identity") # Violated!
...
cox.zph(coxM2, transform="identity") # Check
cox.zph(coxF2, transform="identity") # Check
